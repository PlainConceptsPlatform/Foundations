---
title: How to
description: "How Platform runs AI agents: GitHub Agentic Workflows on ubuntu-latest against the Forge gateway is the recommendation, with agent-harness installing the Platform Harness, and loop-task as the fallback."
---
...
Alongside that recommendation, Platform maintains four tools. They put our harness into a codebase,
supply the workflows that run it, cover the cases Actions cannot, and measure the result. All four
...
```text
agent-harness                    agentic-workflows            loop-task
(install the harness)            (run agents on events)       (fallback: run on a clock)
        │                                │                            │
        ▼                                ▼                            ▼
  npx @plainconceptsplatform/      npx @plainconceptsplatform/   loop-task new 30m --
  agent-harness@latest             workflows                     opencode run "..."
  /repo-initialize
```
...
3. **Add a loop only where an event will not do.** Health checks, data syncs, deploy polls, or
   anything a runner cannot reach.

Each tool holds its own: agent-harness gets the harness into a repository in the first place,
agentic-workflows runs the agents against it, and loop-task works with any shell command rather than
only with AI.
