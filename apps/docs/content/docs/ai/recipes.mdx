...
The production recipes below use GitHub issue labels as a state machine. A label marks **where an issue is in the loop's lifecycle**, and each loop only picks issues with its entry label. This keeps multiple loops running on the same repo without colliding.

### Labels by workflow

| Workflow | Labels | Entry | In-progress | Exit |
|---|---|---|---|---|
...
| **Audit** | `audit:report`, `audit:doing`, `audit:closed`, `bug` | (auto-generated) | `audit:doing` | `audit:closed` |
...
<Mermaid chart={String.raw`stateDiagram-v2
    ...
    code_doing --> code_done: merged + closed
    code_doing --> code_review: blocked or failed
...
These power the [Numa](https://github.com/PlainConceptsPlatform/Numa) project. They are the best reference for building real loop-engineering pipelines: multi-task chains with recovery, retry caps, AI agents, and GitHub label state machines.

### Dev loop

The flagship. Every 20 minutes, picks the oldest `code:pick` issue, implements it end-to-end with an AI agent, verifies (lint + typecheck + tests + build for both frontend and backend), opens a PR, waits for CI, fixes CI failures, merges, and closes the issue. 15 tasks covering the full delivery lifecycle.
...
Every 20 minutes, picks a `refine:pick` issue and rewrites it as a grounded user story (Mike Cohn format + Gherkin acceptance criteria + Mermaid diagram), running `@humanizer` on the prose. Ends with `refine:done` or `refine:questions`.
...
Companion to the refine loop. When a product owner answers clarifying questions (`refine:answers`), this loop picks the issue, reads **only the issue owner's comments** (ignoring others as a security measure), and re-refines the story with the answers.
