...
<Callout type="warn" title="Check the recommendation first">
  ...
  event, the Platform default is [Agentic Workflows](/docs/ai/workflows) on a self-hosted runner,
  and these patterns translate to event triggers rather than intervals. The label scheme is
  simpler there: an event carries the transition, so the states do not need namespacing.
</Callout>
