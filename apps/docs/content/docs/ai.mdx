---
title: AI x SDLC
description: "AI across the Platform delivery lifecycle on one page: GitHub Agentic Workflows on ubuntu-latest against the Forge gateway, agent-harness installing the Platform Harness, project-health measuring the result, and the full workflow contract."
---
...
The workflow shape and catalog are covered in [Agentic workflows](#agentic-workflows), further down
this page. What to do with the metrics is in
[Analytics with DORA](/docs/analytics).
...
- **Our gateway and our model, not a vendor's.** gh-aw ships an `opencode` engine, pointed at
  ...
  model: openai/quasar-438b   # `openai` only satisfies gh-aw's validation
  ```
...
The tools compose, and each one still holds its own. agent-harness gets the harness into a
repository in the first place, agentic-workflows runs the agents against it, and project-health
...
The maintained catalog lives in
[**PlainConceptsPlatform/agentic-workflows**](https://github.com/PlainConceptsPlatform/agentic-workflows),
which is public. It ships the routes, the workers, the composite actions, and the CLI that installs
and updates them, so a repository does not copy these by hand:

```bash
npx @plainconceptsplatform/workflows
```

That opens a TUI listing every route and template with its install status. Installing any route brings
the whole managed set: actions, workers, the router, the compile script, and the mandatory
`opencode.ci.json`. The router, classifier, and route matrix are
[derived from the installed route set](https://github.com/PlainConceptsPlatform/agentic-workflows/blob/main/docs/ownership.md),
so adding or removing a route regenerates them and the router always references exactly the workers on
disk.

Install the harness first: the workers call the skills and commands
[agent-harness](https://github.com/PlainConceptsPlatform/agent-harness) provides.

The full route set is `refine`, `implement`, `direct`, `apply-review`, `merge-gate`,
`visual-verify`, `audit`, `propose`, `release`, and `triage`, each a
[worker in `loops/workflows/`](https://github.com/PlainConceptsPlatform/agentic-workflows/tree/main/loops/workflows).
Two of them are documented below:
...
work-router.yml       every on: the repository has
  └── classify        one event in, exactly one route out
        ...
        ├── call-apply-review   → apply-review.lock.yml    workflow_call only
        ├── call-merge-gate     → merge-gate.lock.yml      workflow_call only
        ├── call-visual-verify  → visual-verify.lock.yml   workflow_call only
        ├── call-audit          → audit.lock.yml           workflow_call only
        ├── call-release        → release.lock.yml         workflow_call only
        ├── call-triage         → triage.lock.yml          workflow_call only
...
Reaching for `args: ["--model", "plainconcepts/quasar-438b"]` to be explicit about the model is a
...
gh-aw's own `awf-proxy/quasar-438b` either way. Verified on gh-aw v0.83.4 by compiling both.
...
workflow and check the references in CI. We no longer use `workflow_run` at all: it is not
delivered for token-created runs, which is every bot pull request; see
[draining a queue without polling](#draining-a-queue-without-polling) for what replaced it.
...
for a result; CI reporting its own conclusion as an event means the polling and the
...
dispatch-merge-gate:
  needs: [api, web, secret-scan, deps-and-iac-scan, sast-scan, sbom]
  if: always() && github.event_name == 'pull_request' && endsWith(github.event.pull_request.user.login, '[bot]')
  steps:
    - uses: actions/create-github-app-token@… # the bot App, not GITHUB_TOKEN
    - run: gh workflow run work-router.yml -f operation=merge-gate -f pr-number=… -f ci-conclusion=…
```

An earlier version of this recipe used a `workflow_run` trigger on the router to hear CI
completions. It looked right, worked in every manual test, and silently never fired in
production. The trap: **GitHub does not deliver `workflow_run` (or most other events) for
workflow runs whose originating event was created with `GITHUB_TOKEN`** — and every bot pull
request is token-created, so its CI runs are invisible to `workflow_run` listeners. Manual tests
pass because a human touching the PR becomes the actor, which un-poisons the chain. The reliable
shape is the one above: CI's own final job reports its conclusion by `workflow_dispatch`, minted
with the bot App token — `workflow_dispatch` and `repository_dispatch` are the two events exempt
from the cascade rule.

Two details that still bite:

- **`if: always()`** on the dispatch job, or a failing check swallows the report and the gate
  never hears about exactly the runs it exists to fix.
- **A cron reconciler as the belt.** Dispatches can be lost to an outage or a cancelled run; a
  scheduled sweep that compares each open bot PR's newest CI run against the newest gate verdict
  re-dispatches anything the event path dropped.

### A feature is many issues, one branch, one pull request

A feature issue lists its stories as `#123` mentions in the body, in execution order. Adding the
`feature` label starts a chain: the first link creates an integration branch and records it as a
hidden marker in the feature body — `<!-- feature-branch: feature/39-... -->` — so every later
link, and any resume after a crash, re-derives the branch from GitHub instead of remembering it.

Each story is then implemented **on top of the previous ones**: the worker starts from the branch
head, commits in its sandbox, and the workflow replays the patch onto the real branch with the
bot identity. No pull request and no CI per story, by design: cost and noise scale with features,
not stories. When no open story remains, a finish pass reviews the accumulated diff, fixes what
the stories broke in combination, writes the feature's one changelog entry, and opens the single
pull request that CI and the merge gate actually judge. A partial feature (skipped stories) opens
as a draft, which nothing auto-merges.

The same marker idea links ordinary issues to their pull requests: implement stamps
`<!-- implement-pr: N -->` and `<!-- implement-branch: ... -->` on the issue when the PR is born,
so the duplicate check is an exact lookup (an open marker PR blocks a second implement; a merged
or closed one allows a legitimate redo), and anything acting on the issue later knows the branch
without guessing. Removing the `feature` label stops a chain; adding it back resumes idempotently.
