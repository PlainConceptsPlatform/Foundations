...
- **`lint:md` is a sibling script to `lint`**, folded into the root `lint` composite and into `lint-staged` for staged `.md` files, the same way Biome is wired for `.ts`/`.tsx`.
...
Copy `.markdownlint-cli2.jsonc` and `.markdownlint-rules.cjs` from this repo's root, or from any Platform app that already has them. There is no installer for this yet: the harness deliberately does not patch a consumer's `package.json` or hooks, so wiring it up is a five-minute hand edit, once, the same way it was done here.
